              ...
              <Card className="overflow-hidden">
                <div className="flex items-center justify-between border-b border-border p-4">
                  <div>
                    ...
                    <p className="text-sm text-muted">Visual simples: arquivos aparecem pelo nome/miniatura. URLs aparecem apenas para sites. Links podem usar modo automático, proxy para sistema próprio ou link externo.</p>
                  </div>
                    ...
                    <tbody className="divide-y divide-border/70">
                      {slides.map((slide, index) => (
                        <tr key={slide.id}>
                          ...
                          <td className="px-4 py-3">{(slide.type === "URL" || slide.type === "DASHBOARD") ? <Select value={slide.openMode} onChange={(event) => patchSlide(slide.id, { openMode: event.target.value as EditableSlide["openMode"] })}><option value="IFRAME">Automático</option><option value="PROXY">Sistema próprio / proxy</option><option value="NEW_TAB">Link externo</option></Select> : <span className="text-xs text-muted">Arquivo</span>}</td>
